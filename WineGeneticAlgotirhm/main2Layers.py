...
import csvReader
import pygame
from neuralNetwork2Layers import forward_pass, fitness_func_factory, on_generation
import csv
...
X_norm = (X - min_values) / (max_values - min_values + 1e-8)
...
num_weights = (input_size * hidden_size1) + hidden_size1 \
            + (hidden_size1 * hidden_size2) + hidden_size2 \
            + (hidden_size2 * output_size) + output_size

fitness_func = fitness_func_factory(X_norm, Y, input_size, hidden_size1, hidden_size2, output_size)
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            if button_rect.collidepoint(event.pos):
                ...
                output = forward_pass(best_solution, arr, input_size, hidden_size1, hidden_size2, output_size)
